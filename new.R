...
library(shinyjs)
library(writexl)
library(XLConnect)  # For SAS xpt export support
...
convert_to_other_format <- function(data, target_format, file_path) {
  tryCatch({
    ...
    result <- switch(tolower(target_format),
                     ...
                     "sas7bdat" = write_sas(export_data, file_path),
                     "xpt" = write_xpt(export_data, file_path),
...
# Function to write SAS xpt (transport) file for JMP compatibility
write_xpt <- function(data, file_path) {
  # Ensure .xpt extension
  if (!grepl("\\.xpt$", file_path)) {
    file_path <- paste0(file_path, ".xpt")
  }
  
  # Use haven to write xpt file (SAS transport format)
  haven::write_xpt(data, file_path, version = 8)
  return(TRUE)
}

# Function to write SAS sas7bdat file (standard SAS dataset)
write_sas <- function(data, file_path) {
  # Ensure .sas7bdat extension
  if (!grepl("\\.sas7bdat$", file_path)) {
    file_path <- paste0(file_path, ".sas7bdat")
  }
  
  # Use haven to write sas7bdat file
...
server <- function(input, output, session) {
  ...
  # NEW: Convert to other format (bidirectional) - UPDATED for SAS options
  ...
  # Download handler for other formats - UPDATED for SAS .sas7bdat and .xpt
