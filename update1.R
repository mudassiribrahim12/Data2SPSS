...
convert_to_other_format <- function(data, target_format, file_path) {
  tryCatch({
    ...
    for (i in seq_along(export_data)) {
      ...
      if (inherits(col, "haven_labelled")) {
        ...
        if (!is.null(labels) && length(labels) > 0) {
          # Create factor with labelled values
    ...
    result <- switch(tolower(target_format),
                     ...
                     "xlsx" = write_xlsx(export_data, file_path),
                     "dta" = write_dta(export_data, file_path),
                     "sav" = write_sav(export_data, file_path),
                     ...
                     "sas" = write_sas(export_data, file_path),
                     "json" = write_json(export_data, file_path),
...
ui <- fluidPage(
  ...
  useShinyjs(),
  title = "Data2SPSS - Professional Data Conversion to SPSS Tool",
  titleWidth = 350,
  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      /* Improved card typography - FIXED OVERFLOW */
      .blog-card {
        background-color: white;
        padding: 1.8rem; /* Reduced to fit content better */
        border-radius: 12px;
      ...
      /* FIXED: Info box with better overflow handling */
      .info-box {
        background-color: white;
      ...
      /* FIXED: Improved buttons with better text - NOW FIT PROPERLY */
      .btn-hopkins-primary {
        background: linear-gradient(135deg, var(--hopkins-blue) 0%, var(--hopkins-dark-blue) 100%);
        border: none;
        color: white;
        ...
        border-radius: 6px;
        transition: all 0.3s ease;
        padding: 12px 20px; /* Reduced for better fit */
        font-size: 1.1rem; /* Reduced */
      ...
      .process-step {
        position: relative;
        padding: 1.5rem 1.5rem 1.5rem 80px; /* Reduced */
        margin-bottom: 1.2rem;
        background: white;
        border-radius: 8px;
      ...
      .dark-theme .data-preview-container {
        border-color: #444;
      }
      ...
      /* FIXED: Form controls with better font sizes */
      .form-control {
        font-size: 1.1rem; /* Reduced */
        padding: 10px 12px;
        background-color: white;
      ...
      .dark-theme .shiny-input-container {
        color: var(--text-primary);
      }
      
      /* Shiny file input styling */
      ...
      .dark-theme .welcome-section {
        background: linear-gradient(135deg, #1A2530 0%, #252525 100%);
      }
      ...
      /* FIXED: Column layout for Data Converter tab */
  ...
  div(class = "blog-header",
      div(class = "blog-header-content",
          ...
          p(class = "app-subtitle-main", 
            "Professional Data Conversion to SPSS Tool"),
          p(class = "app-tagline", 
  ...
  div(class = "container-fluid",
      ...
      tabsetPanel(
        ...
        # Tab 2: Data Converter (main functionality) - FIXED LAYOUT
        tabPanel(
          "Data Converter",
        ...
        # NEW TAB 4: Export to Other Formats (Bidirectional Conversion)
        tabPanel(
                    ...
                    div(class = "conversion-section",
                        ...
                        div(class = "info-box",
                            ...
                            tags$ul(
                              ...
                              tags$li("SPSS - IBM SPSS Statistics (.sav)"),
                              tags$li("R Data - R data file (.rdata)"),
                              tags$li("SAS - SAS dataset (.sas7bdat)"),
                              tags$li("JSON - JavaScript Object Notation"),
                        ...
                        div(class = "step-box",
                            h4("💡 How It Works:"),
                            p("The tool automatically handles labelled variables from SPSS and Stata, converting them to appropriate formats (factors with labels) for export. All value labels and variable names are preserved where possible.")
                        ),
  ...
  div(class = "copyright-footer",
      p(paste("Copyright © 2025-", format(Sys.Date(), "%Y"), "Data2SPSS | Developed by Mudasir Mohammed Ibrahim")),
...
server <- function(input, output, session) {
  ...
  # NEW: Convert to other format (bidirectional)
  observeEvent(input$convert_other_btn, {
    ...
    tryCatch({
      showModal(modalDialog(
        div(
          style = "text-align: center;",
          h4(paste("Converting to", toupper(target), "format...")),
          div(class = "loading-spinner"),
          p("Preparing data for export...")
        ), 
        footer = NULL
      ))
      
      # Store the data for download (conversion will happen at download time)
      export_ready_data(data_loaded())
      ...
      showNotification(
        paste("✅ Ready to export as", toupper(target), "! Click download to save."),
        type = "message", duration = 6
      )
  ...
  # Download handler for other formats
